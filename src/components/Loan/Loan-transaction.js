...
const LoanTransaction = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.loanTransaction && props.loanTransaction.length > 0) {
      for (let i = 0; i < props.loanTransaction.length; i++) {
        ...
        data.paymentdate = Util.dateFormat(
          props.loanTransaction[i].paymentdate
        );
  ...
  useEffect(() => {
    if (props.loanDetails && props.loanDetails.length > 0) {
      ...
      console.log("conaole_today", Util.getCurrentDate("-"));
  ...
  const showTableView = () => {
    ...
    if (valid) {
      ...
      if (props.loanDetails) {
        ...
        var payment = "";
        let roi = parseFloat(
          loanData[0].renewal_intrest_rate
            ? loanData[0].renewal_intrest_rate
            : rateofinterest
        );
        var mroi = roi / (12 * 100);
        ...
        for (let i = 0; i < months; i++) {
          ...
          if (i == 0) {
            payment =
              loa /
              ((Math.pow(1 + mroi, term) - 1) /
                (mroi * Math.pow(1 + mroi, term)));
            loanamount = loa;
            payment = payment;
            interest = loa * mroi;
            ...
            console.log("console_paymentdate", paymentdate);
          }

          data.month = i + 1;
          data.loanamount = Util.addCommasList(
          ...
          data.extra = "";
          data.endingloan = Util.addCommasList(
            parseFloat(endingloan).toFixed(2)
          );
        ...
        <div className="row">
          ...
          <div className="col-md-8 house-form ">
            <span className="" id="text">
              Let's generate all the transaction for this loan you have paid
              till date.
            </span>
            <br></br>
            <br></br>
