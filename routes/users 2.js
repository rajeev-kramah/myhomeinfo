	...
	con.connect(function(err) {
		...
		con.query(sql, function (err, user) {
		 ...
		 if (user.length > 0) {
				...
				// 	}
				// 	hash_pass = hash;
					 sql = "INSERT INTO owner (email, username,firstname,lastname,role,renewalDate,spaceUsage, substartdate, subenddate, mono, password,account_status) VALUES ('"+email+"', '"+username+"','"+firstname+"','"+lastname+"','"+role+"','"+renewalDate+"','"+spaceUsage+"','"+substartdate+"', '"+subenddate+"', '"+mono+"', '"+password+"','"+account_status+"' )";
					con.query(sql, function (err, user) {
						if (err) {
							res.send(
								result.response(
 ...
 */
 router.post("/userdeatils", async (req, res) => {
	if (!req.body.id) {
		...
		con.connect(function(err) {
			var sql = "SELECT id, name, email, username, address, zipcode, refferedby,maxProperty, substartdate, subenddate,mono,account_status,payment_date,payment_amount  FROM owner where id ='"+req.body.id+"'";
			con.query(sql, function (err, user) {
				if (err) {
					res.send(
						result.response(
	...
	con.connect(function(err) {
		...
		con.query(sql, function (err, user) {
			if (err) {
				...
				sql =  "SELECT id, name, email, username, address, zipcode, refferedby, maxProperty , substartdate, subenddate, mono ,account_status,payment_date,payment_amount FROM owner WHERE id='"+id+"'"
