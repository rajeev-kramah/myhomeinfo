...
router.post("/", async (req, res) => {
  ...
  let bucket_folder_name = req.body.bucket_name;
  con.connect(function (err) {
    ...
    con.query(sql, function (err, user) {
      ...
      if (user.length > 0) {
        ...
        // 	}
        // 	hash_pass = hash;
        sql = "INSERT INTO owner (name, email, username, address, zipcode, refferedby, substartdate, subenddate, mono, password,account_status,payment_amount,payment_date,country,bucket_folder_name) VALUES ('" + name + "', '" + email + "', '" + username + "', '" + address + "', '" + zipcode + "', '" + refferedby + "','" + substartdate + "', '" + subenddate + "', '" + mono + "', '" + password + "','" + account_status + "' ,'" + payment_amount + "' ,'" + payment_date + "' ,'" + country + "','" + bucket_folder_name + "' )";
        con.query(sql, function (err, user) {
          console.log("sql::", sql)
          if (err) {
            res.send(
  ...
  con.connect(function (err) {
    ...
    con.query(sql, function (err, user) {
      if (user.length > 0 && id === '') {
        ...
        sql = "INSERT INTO owner (email, username,name,lastname,maxProperty,role,renewalDate,spaceUsage, substartdate, subenddate, mono, password,account_status, country,bucket_folder_name) VALUES ('" + email + "', '" + username + "','" + name + "','" + lastname + "','" + maxProperty + "','" + role + "','" + renewalDate + "','" + spaceUsage + "','" + substartdate + "', '" + subenddate + "', '" + mono + "', '" + password + "','" + account_status + "','" + country + "','" + bucket_folder_name + "' )";

        if (id) {
          sql = "UPDATE owner SET email= '" + email + "',username= '" + username + "',name= '" + name + "',lastname= '" + lastname + "',maxProperty= '" + maxProperty + "',role= '" + role + "',renewalDate= '" + renewalDate + "',spaceUsage= '" + spaceUsage + "',substartdate= '" + substartdate + "',subenddate= '" + subenddate + "',mono= '" + mono + "',password= '" + password + "',account_status= '" + account_status +  "',country= '" + country + "' WHERE id = '" + id + "'";
        }
        con.query(sql, function (err, user) {
          if (err) {
            res.send(
...
router.post("/login", async (req, res) => {
  if (!req.body.email) {
    ...
    con.connect(function (err) {
      ...
      var sql = "SELECT owner.id, name, email, username, password, subenddate,role ,country ,bucket_folder_name from owner where email = '" + email + "' AND password='" + password + "'";
      ...
      con.query(sql, function (err, user) {
        if (err) {
          ...
          sql = "SELECT substartdate, subenddate ,role, account_status from owner where email = '" + email + "'";
          con.query(sql, function (err, user) {
            if (err) {
              res.send(
                result.response(
              ...
              let status = user[0]['account_status'];
              ...
              if (status === 'Expired') {
                res.send(
                  result.response(
                    404,
                    {},
                    "Your account is not active. Kindly contact administrator."
                  )
                );
