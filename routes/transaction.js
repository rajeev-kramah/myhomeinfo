    ...
    if (add_to_warranty == 1) {
      ...
      con.query(sql, function (err, transactions) {
        if (err) {
          ...
          var sql = "SELECT transactions.id, account_name, date, contact_person, type, amount,  principal, interest, comments,receipt, created_at, entered_by, contacts.groupname,contacts.companyname From transactions INNER JOIN contacts ON transactions.account_name = contacts.id where transactions.is_deleted = 0 and transactions.house_id='" + req.body.house_id + "'";
          con.query(sql, function (err, transactions) {
            if (err) {
              res.send(
                result.response(
...
router.post("/gettransactionsAllData", async (req, res) => {
  if (!req.body.house_id) {
    ...
    con.connect(function (err) {
      ...
      var sql = "SELECT transactions.id, account_name, date, contact_person, type, amount, comments,receipt, created_at, entered_by, contacts.groupname,contacts.companyname From transactions INNER JOIN contacts ON transactions.account_name = contacts.id where is_deleted = 1 and transactions.house_id='" + req.body.house_id + "'";
      con.query(sql, function (err, transactions) {
        if (err) {
          res.send(
            result.response(
...
router.post("/delete", async (req, res) => {
  if (!req.body.house_id) {
    res.send(result.response(422, "", "house id is empty"));
  } else {
              ...
              if (err) {
                ...
                con.query(sql, function (err, loanTransaction) {
                  if (err) {
                    ...
                    transactionData = [...transactions, ...loanTransaction]
                    res.send(result.response(200, transactionData, "Transaction deleted Successfully!"));
                  }
                });
  ...
  if (!req.body.house_id) {
    ...
    con.connect(function (err) {
      ...
      con.query(sql, function (err, transactions) {
        ...
        // 					}else {	
        // 						transactionData = [...transactions, ...loanTransaction]
        // 						res.send(result.response(200, transactionData, "Transaction Undeleted successfully!"));
        // 					}
        // 				});
