		...
		con.connect(function (err) {
			...
			con.query(sql, function (err, loan) {
				if (err) {
					...
					sql = "SELECT id,loantype,lname,loannumber,loanamount,rateofinterest,loanterm,mortgage,propertytax,loanbegindate,loanclosuredate,status, additionaldetails, document, downpayment, escrow, escrowPayee, escrowamount, escrowcontact_id, laddress, lcontact_id, lcontactperson, lemail, lphno, lurl, propertytaxPayee, propertytaxcontact_id, purchaseprice From loan where house_id='" + req.body.house_id + "'";
					con.query(sql, function (err, loan) {
						if (err) {
							res.send(
								result.response(
